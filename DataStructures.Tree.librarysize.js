        ...
        if (!matchFunction){
            if (typeof(value) != "string") {
                ...
                matchFunction = function(nodeToMatch, valueToMatchOn) {
                    return (nodeToMatch.Path == value.Path);
                };
            }
            else {
                // default string match is a function that matches by name;
                matchFunction = function(nodeToMatch, valueToMatchOn) {
                    return (nodeToMatch.Path == valueToMatchOn);
                };
            }
        }
...
DataStructures.Tree.createFromFlatTable = function(flatTable){
    ...
    var tree,
        isNull = function(value){
            ...
            return (value == null || value == "null" || value == "NULL" || value == "(Empty)" || value.length == 0 )
        };
    ...
    if (flatTable) {
        if (flatTable instanceof Array) {
            ...
            for (var i=0; i < flatTable.length; i++) {
               ...
               if (isNull(rowToConvertToNode.Parent)) {
                   ...
                   var matchById = function(nodeToMatch, valueToMatchOn) {
                       ...
                       parentNode = tree.findNode(rowToConvertToNode.Parent, matchById);
                    ...
                    /*
                        ...
                        throw {
                            name : "NodeNotFound",
                            message : "unable to find a parent node for row " + rowToConvertToNode.Path
                        }
                    }
                    */
                    else {
